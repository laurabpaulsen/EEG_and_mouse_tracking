...
for(i in 1:length(unique(data_filtered$trial_number))){
  ...
  newDat <- tibble(ID = data$ID[1], 'rt' = data$rt[1], 'left_img' = data$left_img[1], 'right_img' = data$right_img[1], "trial_number" = data$trial_number[1], 'trial_type' = data$trial_type[1], "correct_resp" = data$correct_resp, 'onset_image' = data$onset_img, 'time_clicked' = data$key_t, 'onset_word' = data$onset_word, 'xpos' = xpos, 'ypos' = ypos, 'trial_timestamps' = trial_timestamps)
...
##### Getting derivatives
```{r}
incmouse <- mt_derivatives(incmouse)
incmouse <- mt_measures(incmouse)

# Getting the measures from the mouse
incmeasures <- tibble(incmouse$measures)

# Getting the data from the mouse object
incdata <- tibble(incmouse$data)

# Merging them on ID
merged <- merge(incdata, incmeasures, by = 'mt_id')
...
event_inc <- event_inc %>% select(trial_number,acc_min_time, acc_max_time, onset_image, time_clicked)

write_csv(event_inc, "Stroop_mouse_EEG_data/additional_triggers.csv")
```
